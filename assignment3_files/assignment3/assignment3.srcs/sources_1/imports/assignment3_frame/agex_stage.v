...
assign pctarget_AGEX = PC_AGEX + 4 + 4 * sxt_imm_AGEX;//(sxt_imm_AGEX < 0) ? (PC_AGEX + 4 + ~(~(sxt_imm_AGEX)<<<2)) : (PC_AGEX + 4 + sxt_imm_AGEX<<<2);//(sxt_imm_AGEX < 0) ? (PC_AGEX - (1 <<< -sxt_imm_AGEX)) : (PC_AGEX + (1 <<< sxt_imm_AGEX));
